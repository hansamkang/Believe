	...
	void Update () {
		if(phase[0]) //페이지 1
        {
            ...
            else if (chasing) // 추적
            {
                ...
                if ((positionDuration < -4 || positionDuration > 4)) //플레이어가 보스의 공격 사정거리 밖에 있음
                {
                    rangeIn = false;
                    enemyTransform.position = Vector2.MoveTowards(enemyTransform.position, playerVectorX, moveSpeed);
                }
                else if (positionDuration < 4 && positionDuration > -4 && !rangeIn) //플레이어가 사정거리 안에 있음
                {
                    rangeIn = true;
                    StartCoroutine(Attack(0));
                }
