...
public class PlayerMovement : MonoBehaviour {
    ...
    public float enableJump = 0.9f; //점프 가능 시간
    ...
    float jumpForce = 90f;
    float maxSpeed = 10f;
    float jumpdecrement = 0.1f; //점프 가능 시간 감소량
    ...
    {
        if (collider.gameObject.tag == "EnemyAttack" && !matchless && FindObjectOfType<HeartSystem>().curHealth > 0) //피격시
        {
